@extends('admin.layouts.app', ['title' => 'Edit Monthly Expenses'])
@section('panel')
    ...
    <form action="{{ route('admin.monthlyexpense.update', $monthlyExpense->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">@lang('Edit Monthly Expenses') </h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="pb-3 col-md-12">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" required
                                value="{{ old('name', $monthlyExpense->name) }}">
                        </div>
                    </div>
                    <div class="pb-3 col-md-12">
                        <div class="form-group">
                            <label class="form-label text-capitalize">@lang('Description')</label>
                            <textarea class="form-control" rows="5" name="description">{{ old('description', $monthlyExpense->description) }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
