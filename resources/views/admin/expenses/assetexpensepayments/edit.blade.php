@extends('admin.layouts.app', ['title' => 'Edit Asset Expenses Payment'])
@section('panel')
    ...
    <form action="{{ route('admin.assetexpensepayment.update',$assetexpensepayment->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">@lang('Edit Asset Expense Payment') </h5>
            </div>
            <div class="card-body">
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select" name="asset_expense_id" required>
                                ...
                                @foreach ($assetexpenses as $item)
                                    <option {{ $assetexpensepayment->asset_expense_id == $item->id ? 'selected' : '' }}
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$assetexpensepayment->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-6 pb-3">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="date" name="date"
                                value="{{ $assetexpensepayment->date  }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 pb-3 ">
                        <div class="form-group">
                            <label class="form-label text-capitalize">@lang('Amount')</label>
                            <input class="form-control" type="text" name="amount" value="{{ $assetexpensepayment->amount }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-12">
                        <div class="form-group">
                            <label class="form-label text-capitalize">@lang('Note')</label>
                            <textarea name="note" id="note" class="form-control">{{ $assetexpensepayment->note }}</textarea>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $assetexpensepayment->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
