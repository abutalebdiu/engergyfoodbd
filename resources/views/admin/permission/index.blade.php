...
@section('panel')
    <form action="{{ route('admin.permissions.update') }}" method="POST">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="form-group mb-3">
                                            <div class="d-flex flex-wrap gap-1 align-items-center">
                                                <label
                                                    class="form-label {{ $lastCharacter == '.' ? 'bg-danger' : 'text-cyan' }}">{{ $permission->code }}</label>
                                                <span
                                                    class="badge @if (@$route['method'] == 'GET') bg--success @else bg--warning @endif">
                                                    {{ @$route['method'] }}
                                                </span>
                                            </div>
                                            <input type="hidden" name="permission[{{ $i }}][id]"
                                                value="{{ $permission->id }}">
                                            <div class="input-group w-auto">
                                                <span class="input-group-text">@lang('Name')</span>
                                                <input type="text" name="permission[{{ $i }}][name]"
        ...
        <button type="submit" class="btn btn-primary w-100 mt-3">@lang('Update permission')</button>
