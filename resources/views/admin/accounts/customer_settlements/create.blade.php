@extends('admin.layouts.app', ['title' => 'Add new customer settlement'])
@section('panel')
    <form action="{{ route('admin.customersettlement.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Add New Customer Settlement <a href="{{ route('admin.customersettlement.index') }}"
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="customer_id" id="customer_id" class="form-control" required>
                                ...
                                @foreach (App\Models\User::where('type', 'customer')->where('status', 1)->get() as $customer)
                                    <option  @selected(old('customer_id' == @$customer->id))
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <select name="type" id="type" class="form-control type" required>
                                ...
                                <option value="Minus" {{ old('type') == 'Minus' ? 'selected' : '' }}>Minus</option>
                            </select>
                        </div>
                    </div>
