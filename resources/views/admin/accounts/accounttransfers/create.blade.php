...
@section('panel')
    <form action="{{ route('admin.accounttransfer.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card">
                    ...
                    <div class="pb-3 col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <select name="from_payment_method_id" id="from_payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $methods)
                                    <option data-fromaccounts="{{ $methods->accounts }}" @selected(old('from_payment_method_id' == @$methods->id))
                                        value="{{ $methods->id }}">
                                        {{ $methods->name }} </option>
                                @endforeach
                            </select>
