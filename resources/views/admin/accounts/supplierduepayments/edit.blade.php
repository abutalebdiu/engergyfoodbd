@extends('admin.layouts.app', ['title' => 'Edit Supplier Due Payment'])
@section('panel')
    <form action="{{ route('admin.supplierduepayment.update', $supplierduepayment->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Supplier Due Payment <a href="{{ route('admin.supplierduepayment.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="fa fa-list"></i>Supplier Due Payment
                        List</a>
                </h6>
            </div>
            <div class="card-body">
                <div class="row">
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="supplier_id" id="supplier_id" class="form-select select2">
                                ...
                                @foreach ($suppliers as $supplier)
                                    <option {{ $supplierduepayment->supplier_id == $supplier->id ? 'selected' : '' }}
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$supplierduepayment->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $supplierduepayment->note }}</textarea>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="amount"
                                value="{{ $supplierduepayment->amount }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="date" name="date"
                                value="{{ $supplierduepayment->date }}" required>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $supplierduepayment->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
