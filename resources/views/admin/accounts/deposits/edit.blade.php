@extends('admin.layouts.app', ['title' => 'Edit Deposit Amount'])
@section('panel')
    <form action="{{ route('admin.deposit.update', $deposit->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Deposit Amount <a href="{{ route('admin.deposit.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="fa fa-list"></i>Deposit List</a>
                </h6>
            </div>
            <div class="card-body">
                <div class="row">
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$deposit->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $deposit->note }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Date') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="date" name="date" value="{{ $deposit->date }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="amount" value="{{ $deposit->amount }}"
                                required>
                        </div>
                    </div>
