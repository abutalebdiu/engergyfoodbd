@extends('admin.layouts.app', ['title' => 'Edit Customer Loan'])
@section('panel')
    <form action="{{ route('admin.customerloan.update', $customerloan->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Customer Loan <a href="{{ route('admin.customerloan.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="bi bi-list"></i> Customer Loan List</a>
                </h6>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="title" value="{{ $customerloan->title }}" required>
                        </div>
                    </div>
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$customerloan->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $customerloan->note }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="start_month_id" id="start_month_id" class="form-select">
                                ...
                                @foreach ($months as $item)
                                    <option {{ $customerloan->start_month_id == $item->id ? 'selected' : '' }}
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="year_id" id="year_id" class="form-select">
                                ...
                                @foreach ($years as $year)
                                    <option {{ $customerloan->year_id == $year->id ? 'selected' : '' }}
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="amount" value="{{ $customerloan->amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('interest') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="interest" value="{{ $customerloan->interest }}"
                                required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="total_amount"
                                value="{{ $customerloan->total_amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="monthly_settlement"
                                value="{{ $customerloan->monthly_settlement }}" required>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $customerloan->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
