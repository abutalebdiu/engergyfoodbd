@extends('admin.layouts.app', ['title' => __('Customer Loan Show')])
@section('panel')
    <div class="card">
        <div class="card-header">
            <h6 class="mb-0">@lang('Customer Loan Show')
                <a href="{{ route('admin.customerloan.create') }}" class="btn btn-outline-primary btn-sm float-end"> <i
                        class="bi bi-plus"></i> @lang('Add New Customer Loan')</a>

                <a href="{{ route('admin.customerloan.index') }}" class="btn btn-outline-primary btn-sm float-end me-2"> <i
                        class="bi bi-list"></i> @lang('Customer Loan List')</a>
        ...
        <div class="modal fade" id="LoanPayment" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <form action="{{ route('admin.customerloanpayment.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="customer_loan_id" value="{{ $customerloan->id }}">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">@lang('Customer Loan Payment')</h1>
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-md-6">
                                    <div class="py-2 form-group">
                                        ...
                                        <input type="text" value=" {{ $customerloan->total_amount - $customerloan->customerloanpayments->sum('amount') }}" class="form-control" readonly>
                                    </div>
                                </div>
                                <div class="col-12 col-md-6">
                                    <div class="py-2 form-group">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            <th>@lang('Title')</th>
                            <td>{{ $customerloan->title }}</td>
                            <th>@lang('Date')</th>
                            <td>{{ (Date('d-m-Y', strtotime($customerloan->date))) }}</td>
                        </tr>
                        <tr>
                            <th>@lang('Month')</th>
                            <td> {{ optional($customerloan->month)->name }} - {{ optional($customerloan->year)->name }}
                            </td>
                            <th>@lang('Account')</th>
                            <td>{{ optional($customerloan->account)->title }}</td>
                        </tr>
                        <tr>
                            <th>@lang('Amount')</th>
                            <td> {{ (number_format($customerloan->amount)) }}</td>
                            <th>@lang('Interest')</th>
                            <td> {{ (number_format($customerloan->interest)) }}</td>
                        </tr>
                        <tr>
                            <th>@lang('Total Amount')</th>
                            <td> {{ (number_format($customerloan->total_amount)) }}</td>
                            <th>@lang('Monthly Installment')</th>
                            <td> {{ (number_format($customerloan->monthly_settlement)) }}</td>
                        </tr>
                        <tr>
                            <th>@lang('Note')</th>
                            <td colspan="3"> {{ $customerloan->note }}</td>
                        </tr>
                    </thead>
                </table>
            </div>
