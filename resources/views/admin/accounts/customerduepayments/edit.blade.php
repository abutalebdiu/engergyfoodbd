@extends('admin.layouts.app', ['title' => 'Edit Customer Due Payment'])
@section('panel')
    <form action="{{ route('admin.customerduepayment.update', $customerduepayment->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Customer Due Payment <a href="{{ route('admin.customerduepayment.index') }}"
                ...
                <div class="row">
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="customer_id" id="customer_id" class="form-select select2">
                                ...
                                @foreach ($customers as $customer)
                                    <option {{ $customerduepayment->customer_id == $customer->id ? 'selected' : '' }}
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$customerduepayment->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $customerduepayment->note }}</textarea>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="amount"
                                value="{{ $customerduepayment->amount }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="date" name="date"
                                value="{{ $customerduepayment->date }}" required>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $customerduepayment->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
