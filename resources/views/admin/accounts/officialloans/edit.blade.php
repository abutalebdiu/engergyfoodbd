@extends('admin.layouts.app', ['title' => 'Edit Official Loan'])
@section('panel')
    <form action="{{ route('admin.officialloan.update', $officialloan->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Official Loan <a href="{{ route('admin.officialloan.index') }}"
                ...
                <div class="row">
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="title" value="{{ $officialloan->title }}" required>
                        </div>
                    </div>
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$officialloan->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $officialloan->note }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="start_month_id" id="start_month_id" class="form-select">
                                ...
                                @foreach ($months as $item)
                                    <option {{ $officialloan->start_month_id == $item->id ? 'selected' : '' }}
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="year_id" id="year_id" class="form-select">
                                ...
                                @foreach ($years as $year)
                                    <option {{ $officialloan->year_id == $year->id ? 'selected' : '' }}
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="amount" value="{{ $officialloan->amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('interest') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="interest" value="{{ $officialloan->interest }}"
                                required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="total_amount"
                                value="{{ $officialloan->total_amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="monthly_settlement"
                                value="{{ $officialloan->monthly_settlement }}" required>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $officialloan->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
