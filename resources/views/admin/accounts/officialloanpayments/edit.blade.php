@extends('admin.layouts.app', ['title' => 'Edit Official Loan Payment'])
@section('panel')
    <form action="{{ route('admin.officialloanpayment.update', $officialloanpayment->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Official Loan Payment <a href="{{ route('admin.officialloanpayment.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="fa fa-list"></i> Official Loan Payment
                        List</a>
                </h6>
            </div>
            <div class="card-body">
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$officialloanpayment->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="amount"
                                value="{{ $officialloanpayment->amount }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="date" name="date"
                                value="{{ $officialloanpayment->date }}" required>
                        </div>
                    </div>
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Note') <span
                                    class="text-danger">*</span></label>
                            <textarea name="note" id="note" class="form-control">{{ $officialloanpayment->note }}</textarea>
                        </div>
                    </div>
                </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $officialloanpayment->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
