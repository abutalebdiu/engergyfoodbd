@extends('admin.layouts.app', ['title' => 'Edit Item Order Payment History'])
@section('panel')
    <form action="{{ route('admin.itemorderpayment.update', $itemorderpayment->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Item Order Payment History
                    <a href="{{ route('admin.itemorderpayment.index') }}"
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$itemorderpayment->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="text" name="amount"
                                value="{{ $itemorderpayment->amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-3">
                        <div class="form-group py-2">
                            ...
                            <input class="form-control" type="date" name="date"
                                value="{{ $itemorderpayment->date }}" required>
                        </div>
                    </div>
                </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $itemorderpayment->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
