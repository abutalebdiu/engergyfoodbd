                        ...
                        <div class="form-group">
                            <select name="month_id" id="month_id" class="form-select">
                                ...
                                @foreach ($months as $month)
                                    <option {{ request()->month_id == $month->id ? 'selected' : '' }}
                    ...
                    <tbody>
                        ...
                        @forelse($itemstocks as $category_id => $items)
                            <tr class="bg-secondary">
                                <td colspan="15" class="text-start  text-white">
                                    <strong>{{ optional($items->first()->item->category)->name }}</strong>
                                </td>
                            </tr>

                            @php
                                $categoryTotalStock = $items->sum('current_stock');
                                $categoryTotalValue = $items->sum(
                                    fn($item) => $item->current_stock * $item->item->price,
                                );
                                ...
                                $categoryTotalPhyValue = $items->sum(
                                    fn($item) => $item->physical_stock * $item->item->price,
                                );
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a href="{{ route('admin.itemtstock.edit', $item->id) }}"> <i
