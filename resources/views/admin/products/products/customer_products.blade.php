@extends('admin.layouts.app', ['title' => 'Customer Product Commission'])
@section('panel')
    <form action="{{ route('admin.product.customer.price.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0 text-capitalize">Commission Setup</a>
                    <div class="float-end">
                                ...
                                @foreach ($products as $key => $product)
                                    <tr>
                                        ...
                                        <td class="text-capitalize">
                                            <label class="form-label text-capitalize"> {{ $product->product->name }}</label>
                                            <input type="hidden" name="invoice_id[]" value="{{ $product->id }}">
                                        </td>
                                        <td class="">
                                            <input class="form-control" type="text" name="price[]"
                                                value="{{ en2bn($product->price) }}" required>
                                        </td>
                                        <td class="text-end">
