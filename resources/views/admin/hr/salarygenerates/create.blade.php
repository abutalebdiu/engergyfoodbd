@extends('admin.layouts.app',['title'=>'Add New Salary Proccess'])
@section('panel')
    <form action="{{ route('admin.salarygenerate.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Add New Salary Proccess <a href="{{ route('admin.salarydeduction.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="bi bi-list"></i> Salary Proccess List</a>
                </h6>
            </div>
            <div class="card-body">
                <div class="row">
                  {{--
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Employee')</label>
                            <select name="employee_id" id="employee_id" class="form-select">
                                <option value=""> -- Select -- </option>
                                @foreach ($employees as $item)
                                    <option value="{{ $item->id }}">{{ $item->emp_id }} - {{ $item->name }} - {{ optional($item->department)->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    <script>
        $('[name=department_id]').on('change', function() {
            ...
            $.each(employees, function(index, value) {
                option += "<option value='" + value.id + "' " + (value.id == "" ? "selected" : "") + ">" + value.emp_id +" - "+
                    value.name + "</option>";
            });
