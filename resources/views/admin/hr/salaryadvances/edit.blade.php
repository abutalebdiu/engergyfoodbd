@extends('admin.layouts.app',['title'=>'Edit Advanced Salary'])
@section('panel')
    <form action="{{ route('admin.salaryadvance.update', $salaryadvance->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Advanced Salary <a href="{{ route('admin.salaryadvance.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="bi bi-list"></i>Advanced Salary List</a>
                </h6>
            </div>
            <div class="card-body">
                <div class="row">
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$salaryadvance->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $salaryadvance->note }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="employee_id" id="employee_id" class="form-select">
                                ...
                                @foreach ($employees as $item)
                                    <option {{ $salaryadvance->employee_id == $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="month_id" id="month_id" class="form-select" required>
                                ...
                                @foreach ($months as $item)
                                    <option {{ $salaryadvance->month_id == $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="year_id" id="year_id" class="form-select" required>
                                ...
                                @foreach ($years as $year)
                                    <option {{ $salaryadvance->year_id == $year->id ? "selected" : "" }} value="{{ $year->id }}">{{ $year->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="amount" value="{{ $salaryadvance->amount }}"
                                required>
                        </div>
                    </div>
        ...
        $('[name=payment_method_id]').on('change', function() {
            ...
            $.each(accounts, function(index, value) {
                ...
                option += "<option value='" + value.id + "' " + (value.id ==
                        "{{ $salaryadvance->account_id }}" ? "selected" : "") + ">" +
                    name + "</option>";
            });
