@extends('admin.layouts.app',['title'=>'Edit Loan'])
@section('panel')
    <form action="{{ route('admin.loan.update', $loan->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Edit Loan <a href="{{ route('admin.loan.index') }}"
                ...
                <div class="row">
                            ...
                            <select name="payment_method_id" id="payment_method_id" class="form-control" required>
                                ...
                                @foreach (App\Models\Account\PaymentMethod::with('accounts')->get() as $method)
                                    <option value="{{ $method->id }}" data-accounts="{{ $method->accounts }}"
                                        @selected(old('payment_method_id', @$loan->payment_method_id == @$method->id))>
                                        {{ $method->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-12 col-md-12">
                        <div class="form-group py-2">
                            ...
                            <textarea name="note" id="note" class="form-control">{{ $loan->note }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="start_month_id" id="start_month_id" class="form-select">
                                ...
                                @foreach ($months as $item)
                                    <option {{ $loan->start_month_id == $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group py-2">
                            ...
                            <select name="year_id" id="year_id" class="form-select">
                                ...
                                @foreach ($years as $year)
                                    <option {{ $loan->year_id == $year->id ? "selected" : "" }}  value="{{ $year->id }}">{{ $year->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="amount" value="{{ $loan->amount }}"
                                required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('interest') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="interest" value="{{  $loan->interest }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('total Amount') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="total_amount" value="{{  $loan->total_amount }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group py-2">
                            <label class="form-label text-capitalize">@lang('Monthly Installment') <span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="monthly_settlement" value="{{  $loan->monthly_settlement }}" required>
                        </div>
                    </div>
