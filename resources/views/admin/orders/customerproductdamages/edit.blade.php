@extends('admin.layouts.app', ['title' => __('Edit Customer Damage Product')])
@section('panel')
    <form action="{{ route('admin.customerproductdamage.update', $customerproductdamage->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0 text-capitalize">@lang('Edit Customer Damage Product')

                    <a href="{{ route('admin.customerproductdamage.index') }}"
                        class="btn btn-outline-primary btn-sm float-end"> <i class="fa fa-list"></i> @lang('Customers Products Damage List')</a>
                </h6>
            </div>
            <div class="card-body">
                    ...
                    <div class="pb-3 col-12 col-md-4">
                        ...
                        <select name="customer_id" id="customer_id" class="form-select select2" required>
                            ...
                            @foreach ($customers as $customer)
                                <option {{ $customerproductdamage->customer_id == $customer->id ? 'selected' : '' }}
                    ...
                    <div class="pb-3 col-12 col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="date"
                                value="{{ $customerproductdamage->date }}" required>
                        </div>
                    </div>
                    <div class="pb-3 mb-2 col-12 col-md-8">
                        <div class="form-group">
                            <label class="form-label text-capitalize">@lang('Note')</label>
                            <textarea name="note" id="note" class="form-control">{{ $customerproductdamage->note }}</textarea>
                        </div>
                    </div>
                </div>
                                        ...
                                        @foreach ($chunk as $key => $product)
                                            <tr style="border-bottom: 2px solid #21b2c1;">
                                                ...
                                                <td><input type="text" name="product_qty[]"
                                                        id="product_qty_{{ $key }}" value="{{ en2bn(optional($customerproductdamage->customerproductdamagedetail->where('product_id', $product->id)->first())->qty ?? 0.0) }}"
