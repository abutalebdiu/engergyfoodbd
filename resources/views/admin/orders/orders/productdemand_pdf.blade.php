...
<head>
    ...
    <title>@lang('Product Demand List')</title>
    <style>
        @font-face {
    ...
    <div class="wrapper">
        ...
        <div class="quotation-title">
            ...
            <p style="margin: 0;padding:0;width:100%;text-align: center">
                <span>@lang('Product Order List')</span>
            </p>
        </div>
                    ...
                    <table border="1" style="margin-bottom:20px;">
                        ...
                        <tbody>
                            @foreach ($products as $departmentId => $productGroup)
                                ...
                                @endphp
                                <tr>
                                    <th colspan="{{ 4 + count($customerChunk) }}" style="text-align: left">
                                        @lang('Department'): {{ $departmentName }}
                                    </th>
                                </tr>
                                ...
                                @foreach ($productGroup->groupBy('product_id') as $productId => $productItems)
                                    <tr>
                                        ...
                                        @foreach ($customerChunk as $customer)
                                            @php
                                                $customerOrder = $productItems->where('customer_id', $customer->id)->first();
                        ...
                        <tfoot>
                            <tr>
                                    ...
                                    @php
                                        $totalQtyByCustomer = $products
                                            ->flatMap(function ($productGroup) use ($customer) {
                                                return $productGroup->where('customer_id', $customer->id)->pluck('total_qty');
                                            })
                                            ->sum();
                                    @endphp
                                    @if ($type == 'WC')
                                ...
                                @php
                                    $grandTotalQty = $products->flatMap(function ($group) {
                                        return $group->pluck('total_qty');
                                    })->sum();
    
                                    $grandTotalAmount = $products->flatMap(function ($group) {
