...
<head>
    ...
    <title>@lang('Order Invoice') - {{ $order->oid }}</title>
    <style>
        @font-face {
            ...
            <table>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: left;width:1%">:</td>
                        <td style="text-align: left;width:40%">{{ en2bn(optional($order->customer)->mobile) }}</td>
                    </tr>
                    <tr>
                        ...
                        <td style="text-align: left;width:1%">:</td>
                        <td style="text-align: left;width:40%">{{ en2bn(Date('d-m-Y', strtotime($order->date))) }}</td>
                    </tr>
                    <tr>
        ...
        <div class="product-detail">
            <table border="1">
                    ...
                    @foreach ($order->orderdetail as $odetail)
                        ...
                        <tr style="text-align: center">
                            ...
                            <td style="text-align: right;padding-right:15px">
                                {{ en2bn(number_format($odetail->amount, 2, '.', ',')) }}</td>
                            <td style="text-align: right;padding-right:15px">
                                {{ en2bn(number_format($odetail->product_commission, 2, '.', ',')) }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        ...
                        <td style="text-align: right;padding-right:15px">
                            {{ en2bn(number_format($order->orderdetail->sum('amount'), 2, '.', ',')) }}</td>
                        <td style="text-align: right;padding-right:15px">
                            {{ en2bn(number_format($order->orderdetail->sum('product_commission'), 2, '.', ',')) }}</td>
                ...
                <tfoot>
                    ...
                    <tr>
                        <th colspan="5"></th>
                        <th style="text-align: left">@lang('Return Commission') <span style="color:green">(+)</span></th>
                        <td style="text-align: right;padding-right:15px">{{ en2bn(number_format($order->orderreturn->sum('commission'), 2, '.', ',')) }}</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;padding-right:15px">{{ en2bn(number_format($order->grand_total, 2, '.', ',')) }}
                        </td>
                        <td></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="8">কথায়: {{ $banglanumber }} টাকা মাত্র</td>
                    </tr>
                </tfoot>
            </table>
        </div>
                  ...
                  @if ($order->orderreturn->count() > 0)
                    <div class="card">
                        <div class="card-header">
                            <h5 style="text-align:center;padding:0;margin:0">@lang('Order Return')</h5>
                        </div>
                        <div class="card-body">
                            <table border="1">
                                <thead>
                                        ...
                                        @foreach ($orderreturn->orderreturndetail as $odetail)
                                            @if ($odetail->qty > 0)
                                                <tr>
                                                    ...
                                                    <td>{{ en2bn(number_format($odetail->product_commission,2)) }}</td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    @endforeach
