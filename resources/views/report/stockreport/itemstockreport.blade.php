@extends('admin.layouts.app', ['title' => 'Item Stock Reports'])
@section('panel')
    @include('report.layouts.default', [
        'title' => 'Stock Reports',
        'url' => 'admin.reports.itemstockreport',
        [
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    ...
                                    @foreach($itemswithcategories as $items)
                                        @php
                                       ...
                                       <tr>
                                        ...
                                        @foreach ($items as $item)
                                            @php
                                                $totalvalue += $item->price * $item->stock($item->id);
                                            @endphp
                                            <tr>
                                                <td>{{ en2bn($loop->iteration) }}</td>
                                                <td style="text-align: left">{{ $item->name }}</td>
                                                <td>{{ en2bn($item->price) }}</td>
                                                <td>{{ en2bn(round($item->stock($item->id),2)) }}</td>
                                                <td>{{ en2bn(number_format($item->price * $item->stock($item->id), 2)) }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endforeach
